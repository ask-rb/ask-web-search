...
Patch numbers advance by exactly one per release: `0.6.0` → `0.6.1` →
`0.6.2`. Never skip or jump patch numbers (`0.6.0` → `0.6.3` is wrong), even
...
gemchain guard ask-web-search
gemchain update ask-web-search <new-version> --dry-run
gemchain update ask-web-search <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-web-search.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/web_search/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/web_search/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.6.0` → `0.6.1`.
- Several bug fixes ship together: one release, `0.6.0` → `0.6.1` (never
  `0.6.3`).
- New backwards-compatible API: `0.6.1` → `0.7.0`.
...
- This gem bumped because a dependency released: patch only, e.g. `0.6.1` →
  `0.6.2`, via the gemchain cascade — never hand-published.
